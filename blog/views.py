...
class CatalogViewSet(BaseModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ret = []
        roots = Catalog.objects.filter(parent__isnull=True)
        # print(roots)
        if not roots:
            return Response(ret)
        root: Catalog = roots
        for i in root:
            root_dict = CatalogSerializer(i).data
            root_dict['children'] = []
            ret.append(root_dict)
            parent_dict = {i.id: root_dict}
            # print(ret)
            for cls in i.get_descendants():
                # print(cls)
        ...
        print(ret)
        sorted_ret = sorted(ret, key=lambda x: x['id'],reverse=True)
